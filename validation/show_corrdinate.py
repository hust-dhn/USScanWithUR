...
def visualize_robot_system(T_base_end, T_end_tool, T_end_cam, pcd_in_cam_frame=None):
    """
    可视化机器人系统的各个坐标系和点云
...
if __name__ == "__main__":
    # 1. 模拟数据：假设机器人伸向前方
    # T_base_end: 末端在基座前方 0.5m, 高 0.3m, 绕Y轴旋转了90度(朝下)
    T_base_end = np.eye(4)
    # 旋转矩阵 (绕Y转90度，让Z轴朝前)
    # 这是一个示例，你需要填入你的 forward_kinematics 结果
    T_base_end[:3, :3] = np.array([[ 0,  0,  1],
                                   [ 0,  1,  0],
                                   [-1,  0,  0]]) 
    T_base_end[:3, 3] = [0.5, 0.0, 0.3] 

    # 2. 标定数据 (手眼标定结果)
    # 假设相机装在法兰侧面 10cm 处
    T_end_cam = np.eye(4)
    T_end_cam[:3, 3] = [0.0, 0.1, 0.0] 
    ...
    # 实际使用时: pcd = o3d.io.read_point_cloud("your_local_scan.pcd")
    ...
    # --- 运行可视化 ---
    visualize_robot_system(T_base_end, T_end_tool, T_end_cam, pcd_local)