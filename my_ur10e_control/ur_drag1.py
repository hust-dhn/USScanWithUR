...
def input_target_position():
    ...
    print("提示: 可以按'o'键查看当前位置作为参考")
    
    try:
        user_input = input("请输入目标位置: ")
            ...
            if key.char == 'd':  # 按下 'd' 键开始拖动
                ...
                if new_position is not None:
                    ...
                    if set_custom_target_position(new_position):
                        ...
                        print("设置目标位置失败，请重新输入")
                else:
                    print("未输入有效目标位置，自适应控制未启动")
                
            elif key.char == 'p':  # 按下 'p' 键将当前位置设置为目标位置
                current_pose = rtde_r.getActualTCPPose()
                if set_custom_target_position(current_pose):
                    print("目标位置已设置为当前位置")
