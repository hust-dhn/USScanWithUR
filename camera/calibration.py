...
def main(config_path='config_lc.yaml', image_folder='lc_imgs', robot_poses_file='robot_pos.txt'):
    ...
    T_cam2gripper, T_base2cam_0 = hand_eye_calibration(robot_poses, R_target2cam, t_target2cam)

    print('\nResult: T_cam2gripper (camera -> gripper)')
    print(T_cam2gripper)
    print('\nReference: T_base2cam (camera pose in base frame, computed using first robot pose)')
    print(T_base2cam_0)
    ...
    save_transform_yaml(os.path.join(out_dir, 'T_base2cam_ref.yaml'), T_base2cam_0)
