...
class UR10eKine:
    def __init__(self, robot_ip='192.168.253.101'):
        ...
        self.T_end2tcp = self.load_offset_from_yaml("ur10e/cfg/T_end2tcp.yaml")
    ...
    def IK_wo_end(self, T: np.matrix, curr_theta: list, print_bool: bool=False):
        nx = T[0, 0]
        ny = T[1, 0]
        nz = T[2, 0]
        ox = T[0, 1]
        ...
        if print_bool:
            T1 = self.FK_wo_end(a1)
            T2 = self.FK_wo_end(a2)
            T3 = self.FK_wo_end(a3)
            T4 = self.FK_wo_end(a4)
            T5 = self.FK_wo_end(a5)
            T6 = self.FK_wo_end(a6)
            T7 = self.FK_wo_end(a7)
            T8 = self.FK_wo_end(a8)
        ...
        weights = np.array([1.5, 1.5, 1.5, 0.5, 0.5, 0.5])  # 前三个关节权重为1.5，后三个为1.0

        # 计算加权距离
        weighted_distances = []
        for i in range(8):
            # 将矩阵转换为数组
            sol_array = np.array(solution[i]).flatten()
            curr_array = np.array(curr_theta)
            
            # 计算每个关节的差值（考虑角度周期性）
            diff = sol_array - curr_array
        ...
        opti_sol = solution[closest_index].tolist()[0]
        if (opti_sol[3] - curr_theta[3]) > 3.14:
            opti_sol[3] = opti_sol[3] - 6.28
        elif (opti_sol[3] - curr_theta[3]) < -3.14:
            opti_sol[3] = opti_sol[3] + 6.28

        if (opti_sol[5] - curr_theta[5]) > 3.14:
            opti_sol[5] = opti_sol[5] - 6.28
        elif (opti_sol[5] - curr_theta[5]) < -3.14:
            opti_sol[5] = opti_sol[5] + 6.28
...
def main():
    ...
    # joints_file = 'ur10e/cfg/joints.txt'
    # data = np.loadtxt(joints_file)

    # for i in range(data.shape[0]):
    #     # FK_result = ur10eKine.FK(data[i].tolist())
    #     # print(f"FK结果 {i+1}:\n",FK_result)
    #     print(f"移动至关节位置 {i+1} : {data[i].tolist()}")
    #     ur10eKine.rtde_c.moveJ(data[i].tolist(), 0.2, 0.3)
    #     time.sleep(0.008)
    #     print(f"已移动至关节位置 {i+1} ")
    #     # wait user input to continue
    #     # input("按回车键继续...")

# [ccccccccccccc]  :[[-9.97902143e-01  7.83408732e-03  6.42646030e-02  2.87365079e-01]
#  [-6.47391525e-02 -1.26773684e-01 -9.89816789e-01 -7.98829366e-01]
#  [ 3.92749339e-04 -9.91900731e-01  1.27014903e-01  3.24580834e-01]
#  [ 0.00000000e+00  0.00000000e+00  0.00000000e+00  1.00000000e+00]]
# [PRINT] 关节数据: [1.7214, -1.1876, 1.7825, -0.7227, 1.6559, 3.1303]

    # q = [1.4245, -1.0499, 1.7752, -0.0462, -0.6172, 2.0833]

    q = [1.386858, -1.0129575, 2.1923745, -2.74185314, 0.670338, 4.855135]
     
    for i in range(6):
        if not -3.14 < q[i] <3.14:
            q[i] = q[i] - (q[i]/abs(q[i])) * 6.28

    T_tcp2base_target = np.array([
        [ 0.15556908,  0.46327819 , 0.87245177 , 0.17813595],
        [-0.89074511, -0.31603165 , 0.32664748 ,-0.63762336],
        [ 0.42705047, -0.82794755  ,0.36349811 , 0.28213972],
        [ 0.0        ,  0.0         , 0.0         , 1.0    ]
        ])

    T = ur10eKine.FK_wo_end(q)
    print(f"T: {T}")

    q_temp = ur10eKine.IK_wo_end(T_tcp2base_target, q, True)
    print(f"q: {q_temp}")
    ...
    # for i in range(100):
    #     q[0] -= 0.02
    #     ur10eKine.rtde_c.moveJ(q, 0.2, 0.3)
    #     time.sleep(0.01)
