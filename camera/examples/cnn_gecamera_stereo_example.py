...
from api.shared import StreamType, ChannelDimensions
...
if __name__ == "__main__":
    ...
    def _print_stereo_frame(frame: StereoFrame) -> None:
        print("Right Stereo Frame :")
        _print_image_frame(frame.right_frame)
        print("Left Stereo Frame :")
        _print_image_frame(frame.left_frame)
    ...
    def _print_frame(frame: CnnFrame) -> None:
        ...
        if type(frame) == StereoFrame:
            _print_stereo_frame(frame)
        else:
    ...
    if sys.platform == 'win32':
        cnn_param.network_file_name = "C:/Program Files/Inuitive/InuDev/AI/YOLO_NETS/nu4000_nn_bin_yolov7_tiny.bin"
    elif sys.platform == 'linux':
        cnn_param.network_file_name = "/opt/Inuitive/InuDev/AI/YOLO_NETS/nu4000_nn_bin_yolov7_tiny.bin"
    else:
        raise Exception("This platform doesn't  supported yet.")
    device_params = DeviceParams()
    device_params.fps = 5
    hw_information, dpe  = sensor.init(device_params, cnn_param)
    channelDimensions = ChannelDimensions()
    channelDimensions.height = 16*10
    channelDimensions.width = 16 * 10
    sensor.set_channel_dimensions(4, channelDimensions)
    ...
    stereo_stream = sensor.create_started_stream(StreamType.STEREO, 0, _stereo_callback_func)
    print(f"{type(stereo_stream)} stereo {0} initialization finished")
    cnn_stream = sensor.create_started_stream(StreamType.CNN, "Sout_cnn_0", _cnn_callback_func, cnn_param.network_id,
                                              "cnn_sensor_0")
