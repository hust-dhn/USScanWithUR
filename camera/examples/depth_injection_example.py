...
if __name__ == "__main__":
    ...
    def _print_frame(frame: ImageFrame) -> None:
        if frame is None:
            ...
            print(f"Depth Frame : Format {frame.format}, Height {frame.height}, Width{frame.width}, BytesPerPixel"
    ...
    stereo_channel_id = 0
    map_string_injector_params = hw_information.get_injectors(stereo_channel_id)
    keys = list(map_string_injector_params)
    inject_streamer_name = keys[0]
    inject_resolution = Point2Dim(int(2048 / 2), 960)
    ...
    depth = sensor.create_started_stream(StreamType.DEPTH)
    ...
    if sys.platform == 'win32':
        image_path = r"C:\Program Files\Inuitive\InuDev\Injection\Stereo\Injection_StereoImage.tif"
    elif sys.platform == 'linux':
        image_path = r"/opt/Inuitive/InuDev/Injection/Stereo/Injection_StereoImage.tif"
    else:
        raise Exception("This platform doesn't  supported yet.")
    pixel_array = load_image(image_path)
    for i in range(0, 5000):
        try:
            # _print_frame(injection.inject_file_to_stream(StreamType.DEPTH, image_path))
            start = time.time()
            _print_frame(injection.inject_data_to_stream(StreamType.DEPTH, pixel_array))
            stop = time.time()
