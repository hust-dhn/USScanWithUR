import time
import queue
import threading

from sys_path import *
from api.library_loader import *
from api.errors import ErrorCode, Error
from api.inu_sensor import InuSensor, ConnectionState, DeviceParams, SensorResolution
from api.hw_info import InterleaveMode, ChannelControlParams
from api.stereo import StereoStream, StereoFrame
from api.shared import StreamType

if sys.platform == 'linux':
    import signal

interval = 1.0
test_time = 10
channel = 0
mode = SensorResolution.BINNING
interleave_mode = InterleaveMode.INTERLEAVE
fps = 5

start_time = 0
frame_count = 0

msg_queue = queue.Queue()

# #########################################################  main #####################################################
if __name__ == "__main__":
    def _sensor_callback_func(self, sensor: InuSensor, connection_state: ConnectionState, error: Error) -> None:
        print(f'Sensor callback ConnectionState={connection_state} Error={error}')

    def _print_msg():
        while True:
            msg = msg_queue.get()
            print(f"{msg}")

    def _print_frame(stream: StereoStream, frame: StereoFrame) -> None:
        global start_time, frame_count, msg_queue

        if frame is None:
            print('Invalid stereo frame')
        else:
            # print(
            #     f"\t------------------------------------- Image {frame.frame_index} ---------------------------------")
            # print(f"Frame Type: {type(frame)}, Format: {frame.format}, Width: {frame.width},  Height: {frame.height}, "
            #       f"BytesPerPixel {frame.bytes_per_pixel}")
            # print("\t------------------------------------------------------------------------------------------")

            # print(f'frame index: {frame.frame_index}, left: {frame.left_frame}, right: {frame.right_frame}')
            frame_count += 1
            time_diff = time.perf_counter() - start_time
            if time_diff > interval:
                start_time = time.perf_counter()
                fps = frame_count / time_diff
                # msg_queue.put(f"Frame Type: {type(frame)}, Left Width: {frame.left_frame.width}, Left Height: {frame.left_frame.height}, Right Width: {frame.right_frame.width}, Right Height: {frame.right_frame.height}, FPS: {fps}")
                if interleave_mode == InterleaveMode.LEFT_SENSOR:
                    print(f"Frame Type: {type(frame)}, Channel: {stream.channel_id}, Left Width: {frame.left_frame.width}, Left Height: {frame.left_frame.height}, Right Width: 0, Right Height: 0, FPS: {fps}")
                elif interleave_mode == InterleaveMode.RIGHT_SENSOR:
                    print(f"Frame Type: {type(frame)}, Channel: {stream.channel_id}, Left Width: 0, Left Height: 0, Right Width: {frame.right_frame.width}, Right Height: {frame.right_frame.height}, FPS: {fps}")
                elif interleave_mode == InterleaveMode.INTERLEAVE:
                    print(f"Frame Type: {type(frame)}, Channel: {stream.channel_id}, Left Width: {frame.left_frame.width}, Left Height: {frame.left_frame.height}, Right Width: {frame.right_frame.width}, Right Height: {frame.right_frame.height}, FPS: {fps}")
                else:
                    pass
                frame_count = 0
                start_time = time.perf_counter()

    def _stereo_callback_func(stream: StereoStream, frame: StereoFrame, error: Error) -> None:
        if error is None:
            print(f'Undefined error in {type(stream)}')
        elif error.code != ErrorCode.STATUS_OK:
            print(f'{type(stream)} callback Error = {error.code} {error.description}')
        else:
            _print_frame(stream, frame)

    print(f"Start test application")
    sensor = InuSensor()

    # device_params = DeviceParams()
    # device_params.resolution = ESensorResolution.ESensorResolution(mode)
    # device_params.fps = fps
    # hw_information, dpe = sensor.init(device_params)
    # hw_information.set_channel_interleave(channel, interleave_mode)
    hw_information, dpe = sensor.init()

    for ch in hw_information.channels:
        print(f'channel: {ch}, resolution: {hw_information.channels[ch].ControlParams.SensorRes}, fps: {hw_information.channels[ch].ControlParams.FPS}')

    # the resolution and fps will not take effect, unless set the resolution and fps on all channels
    channel_control_params = ChannelControlParams(hw_information.channels[channel].ControlParams)
    channel_control_params.fps = fps
    channel_control_params.sensor_res = mode
    channel_control_params.interleave_mode = interleave_mode
    channel_control_params_dict = {channel: channel_control_params}
    channels_size, channel_control_params_dict = sensor.start(channel_control_params_dict)
    sensor.register = _sensor_callback_func
    print(f"Sensor initialization finished")

    for ch in channel_control_params_dict:
        print(f'channel: {ch}, resolution: {channel_control_params_dict[ch].sensor_res}, '
              f'fps: {channel_control_params_dict[ch].fps}')

    start_time = time.perf_counter()
    print_thread = threading.Thread(target=_print_msg)
    print_thread.daemon = True
    print_thread.start()

    stream = sensor.create_stream(StreamType.STEREO, channel)
    stream.init()
    stream.start()
    stream.register = _stereo_callback_func
    print("Stereo stream initialization finished")
    print(f'Stereo stream channel: {stream.channel_id}')

    for i in range(test_time):
        try:
            # _print_frame(stream.frame)
            # print(f"iteration {i}")
            time.sleep(1)
        except KeyboardInterrupt:
            break
    print(f"Finish test application")

    stream.register = None
    stream.stop()
    stream.terminate()

    print(f"{type(stream)} Stopped")
    sensor.register = None
    sensor.stop()
    sensor.terminate()
    print(f'Sensor stopped')

    if sys.platform == 'win32':
        os._exit(0)
    elif sys.platform == 'linux':
        os.kill(os.getpid(), signal.SIGINT)
    else:
        sys.exit(0)
