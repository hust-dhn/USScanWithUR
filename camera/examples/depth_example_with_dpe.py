...
from api.inu_sensor import InuSensor, ConnectionState, DeviceParams, DpePath
from api.image_frame import ImageFrame
from api.depth_stream import DepthStream
from api.shared import StreamType
from api.InuStreamsPyth import MapStringString
...
if __name__ == "__main__":
    ...
    for node in dpe:
        print(f"\t\tFile Name  {node.FileName} FrameNum {node.FrameNum}\t")
    ...
    for i in range(0, 5000):
        try:
            ...
            print(type(params))
            if i % 2:
                sensor.update_dpe_params("MIN_DISPARITY_SETUP", DpePath.P1, params)
            else:
                 sensor.update_dpe_params("MIN_DISPARITY_SETUP", DpePath.P1, params)
